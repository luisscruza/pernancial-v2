...
final class CreatePayableRequest extends FormRequest
{
    /**
    ...
    public function messages(): array
    {
        return [
            ...
            'recurrence_day.min' => 'El día de pago debe ser entre 1 y 28.',
            'recurrence_day.max' => 'El día de pago debe ser entre 1 y 28.',
        ];
    }
        ...
        return [
            function (Validator $validator): void {
                ...
                if (! $this->filled('recurrence_day')) {
                    $validator->errors()->add('recurrence_day', 'Selecciona el día de pago para la recurrencia.');
                }
            },
        ];
    }

    public function getPayableDto(): CreatePayableDto
    {
        return new CreatePayableDto(
    ...
    public function getSeriesDto(): ?CreatePayableSeriesDto
    {
        if (! $this->boolean('is_recurring')) {
            return null;
        }

        return new CreatePayableSeriesDto(
            contact: Contact::findOrFail($this->integer('contact_id')),
            currency: Currency::findOrFail($this->integer('currency_id')),
