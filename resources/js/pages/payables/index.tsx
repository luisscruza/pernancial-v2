                            ...
                            <motion.div layout key={dueDate} className="space-y-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                                ...
                                <div className="space-y-3">
                                    {items.map((payable) => {
                                        ...
                                        const progress =
                                            payable.amount_total > 0
                                                ? Math.min(100, Math.round((payable.amount_paid / payable.amount_total) * 100))
                                                : 0;
                                        return (
                                            <div key={payable.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                                                <div className="flex flex-wrap items-start justify-between gap-3">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        {payable.amount_paid > 0 && remaining > 0 && (
                                                            <div className="mt-2">
                                                                <div className="h-1.5 w-32 overflow-hidden rounded-full bg-gray-100">
                                                                    <div className="h-full bg-rose-500" style={{ width: `${progress}%` }} />
                                                                </div>
                                                                <p className="mt-1 text-[11px] text-gray-400">{progress}% pagado</p>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="mt-4 flex flex-wrap items-center gap-2">
                                                    {remaining > 0 && (
                                                        <Button size="sm" variant="default" onClick={() => handleOpenPayment(payable)}>
                                                            Registrar pago
                                                        </Button>
                                                    )}
