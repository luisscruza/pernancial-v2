...
export default function CreatePayable({ contacts, currencies, contactId }: Props) {
    const form = useForm<{
        contact_id: string;
        currency_id: string;
        amount_total: string;
    ...
    return (
        <AppLayout title="Crear CxP">
            <Head title="Crear CxP" />
            <div className="mx-auto w-full max-w-2xl p-4">
                <motion.div className="mb-8 flex items-center gap-4" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                    <Button variant="ghost" size="sm" asChild>
                        <Link href={route('payables.index')} className="gap-2">
                            <ArrowLeft className="h-4 w-4" />
                            Volver
                        </Link>
                    </Button>
                    <div>
                        <h1 className="text-2xl font-semibold text-gray-900">Nueva cuenta por pagar</h1>
                        <p className="text-sm text-gray-500">Registra un pago pendiente o recurrente.</p>
                    </div>
                </motion.div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-3">
                        <Button type="button" variant="outline" asChild className="flex-1">
                            <Link href={route('payables.index')}>Cancelar</Link>
                        </Button>
                        <Button type="submit" className="flex-1 gap-2" disabled={form.processing}>
                            <PlusIcon className="h-4 w-4" />
                            {form.processing ? 'Creando...' : 'Crear CxP'}
                        </Button>
                    </div>
                </form>
            </div>
